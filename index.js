...
townForm.addEventListener('submit', async (e) => {
  ...
  try {
    const { lat, lng } = await fetchTownLocation(town)
    if(displayed) {
 ...
 async function fetchTownLocation(town) {
   const apiUrl = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(town)}&key=${key}`;
